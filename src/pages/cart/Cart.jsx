...
function Cart() {
  ...
  const buyNow = async () => {
    ...
    try {
      const orderRef = collection(fireDB, 'orders'); // Update 'orders' to the name of your Firestore collection
      await addDoc(orderRef, orderInfo);
      toast.success('Order placed successfully');

      // Redirect to the home page using window.location
      window.location.href = '/'; // You can replace '/' with the URL of your home page

      // Optionally, you can clear the cart here as well if needed
    } catch (error) {
      ...
      <div className="h-screen bg-gray-100 pt-5 mb-[60%]" style={{ backgroundColor: mode === 'dark' ? '#282c34' : '', color: mode === 'dark' ? 'white' : '' }}>
        ...
        <div className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0">
          ...
          <div className="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3" style={{ backgroundColor: mode === 'dark' ? 'rgb(32 33 34)' : '', color: mode === 'dark' ? 'white' : '' }}>
            ...
            {cartItems.length > 0 && ( // Render the Buy Now button only when the cart is not empty
              <Modal
                name={name}
                address={address}
                pincode={pincode}
